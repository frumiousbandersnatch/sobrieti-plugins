...
- [X] command: ~undo last~ factoid in channel (~factoids recent/undo~)
- [ ] command: ~forget that~ if have ~op~ cap
- [ ] command: remember quote (requires keeping recent history)
- [ ] gender stuff
- [ ] ~go away~ / ~come back~ (though, really this should be a channel ~+m/-m~ thing!)
- [ ] random trigger
